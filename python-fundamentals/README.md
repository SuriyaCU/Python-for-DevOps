...
* **Validation Rules:**
    * Yield each valid log line. A valid line is any line that is **not empty** and **not a comment**.
    * **Comment Lines:** Lines starting with a `#` character. Whitespace before the `#` should be ignored (e.g., `   # comment` is still a comment).
    * **Whitespace:** Empty lines or lines containing only whitespace should be skipped.
    * **Sanitization:** Each yielded line must have any leading or trailing whitespace removed.
